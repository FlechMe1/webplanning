class Association::MembersController < AssociationController
  before_action :set_member, only: [:show, :edit, :update, :destroy]
  before_action :create_family, only: [:create, :update]

  def index
    @q = @association.members.ransack(params[:q])
    @members = @q.result(distinct: true).paginate(:page => params[:page], :per_page => 25)
  end
  ...
  def create
    @member = @association.members.build(member_params)

    if @member.save
      redirect_to [:association, @member], notice: 'Un nouveau membre a été ajouté'
    else
      render :new
    end
  end
  ...
  def update
    respond_to do |format|
      if @member.update(member_params)
        format.html { redirect_to [:association, @member], notice: 'Membre mis à jour' }
        format.js
      else
  ...
  private
    ...
    def create_family
      if params[:member] && params[:member][:sibling_attributes] && params[:member][:sibling_attributes][:family_id] == 'new_family'
        @family = @association.families.create(name: params[:member][:lastname])
